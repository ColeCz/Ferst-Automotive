...
const AddVehicleSearchCustomer = () => {
  ...
  const handleSearch = async (type, id) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8081/vehicle/search-customers?${searchParams.toString()}`,
        {
          credentials: 'include',
        },
      )
  ...
  const mapDatabaseToFormData = (dbData) => {
    ...
    if (Array.isArray(dbData)) {
      ...
      if (dbData[9]) {
        // SSN is at index 9 for individuals
        return {
          customerId: dbData[0],
          email: dbData[1] || '',
          phone: dbData[2] || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (customerType === 'individual') {
        const [street, city, stateAndZip = ''] = formData.address
          .split(',')
          .map((s) => s.trim())
        const [state_abbrv = '', postal_code = ''] = stateAndZip
          .split(' ')
          .map((s) => s.trim())
        ...
        const [street, city, stateAndZip = ''] = formData.businessAddress
          .split(',')
          .map((s) => s.trim())
        const [state_abbrv = '', postal_code = ''] = stateAndZip
          .split(' ')
      ...
      const response = await fetch(
        `http://localhost:8081/vehicle/add-customer?${queryParams.toString()}`,
        {
          method: 'GET',
          credentials: 'include',
        },
      ...
      if (
        data.message === 'Individual customer added successfully' ||
      ...
      <div className="search-customer-container">
        ...
        {!customerFound && (
          <div className="customer-type-selection">
            ...
            <p>
              If the customer doesn't exist, select a customer type to create
              new:
            </p>
